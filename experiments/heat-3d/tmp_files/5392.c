...
void kernel_heat_3d(int tsteps, int n, double A[120 + 0][120 + 0][120 + 0], double B[120 + 0][120 + 0][120 + 0]) {
  ...
  if (3 <= n) 
    for (t2 = 1; t2 <= 500; t2 += 1) {
      for (t4 = 1; t4 <= n - 2; t4 += 128) 
        for (t6 = t4; t6 <= (n - 2 < t4 + 127 ? n - 2 : t4 + 127); t6 += 1) 
          for (t8 = 1; t8 <= n - 2; t8 += 128) 
            for (t10 = t8; t10 <= (n - 2 < t8 + 127 ? n - 2 : t8 + 127); t10 += 1) 
              for (t12 = 1; t12 <= n - 2; t12 += 64) 
                for (t14 = t12; t14 <= (t12 + 63 < n - 2 ? t12 + 63 : n - 2); t14 += 1) 
