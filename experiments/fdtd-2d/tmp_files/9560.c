...
void kernel_fdtd_2d(int tmax, int nx, int ny, double ex[1000 + 0][1200 + 0], double ey[1000 + 0][1200 + 0], double hz[1000 + 0][1200 + 0], double _fict_[500 + 0]) {
  ...
  for (t2 = 0; t2 <= tmax - 1; t2 += 1) {
    for (t4 = 0; t4 <= ny - 1; t4 += 1) 
      ey[0][t4] = _fict_[t2];
    for (t4 = 1; t4 <= nx - 1; t4 += 30) 
      for (t6 = t4; t6 <= (t4 + 29 < nx - 1 ? t4 + 29 : nx - 1); t6 += 1) 
        for (t8 = 0; t8 <= ny - 1; t8 += 100) 
          for (t10 = t8; t10 <= (ny - 1 < t8 + 99 ? ny - 1 : t8 + 99); t10 += 1) 
            ey[t6][t10] = ey[t6][t10] - 0.5 * (hz[t6][t10] - hz[t6 - 1][t10]);
    for (t4 = 0; t4 <= nx - 1; t4 += 30) 
      for (t6 = t4; t6 <= (t4 + 29 < nx - 1 ? t4 + 29 : nx - 1); t6 += 1) 
        for (t8 = 1; t8 <= ny - 1; t8 += 100) 
          for (t10 = t8; t10 <= (ny - 1 < t8 + 99 ? ny - 1 : t8 + 99); t10 += 1) 
            ex[t6][t10] = ex[t6][t10] - 0.5 * (hz[t6][t10] - hz[t6][t10 - 1]);
    for (t4 = 0; t4 <= nx - 2; t4 += 30) 
      for (t6 = t4; t6 <= (t4 + 29 < nx - 2 ? t4 + 29 : nx - 2); t6 += 1) 
        for (t8 = 0; t8 <= ny - 2; t8 += 100) 
          for (t10 = t8; t10 <= (ny - 2 < t8 + 99 ? ny - 2 : t8 + 99); t10 += 1) 
